...
# 📦 第一步：导入必要的库
# ============================================================================
# 💡 提示：每个库就像是一个"工具箱"，我们需要用到里面的工具

import torch              # PyTorch 核心库 - 深度学习的"瑞士军刀"
import torch.nn as nn     # nn = Neural Network，神经网络模块，包含各种"积木"（层、激活函数等）
import torch.optim as optim  # optim = Optimizer，优化器模块，负责"调整权重"
import numpy as np        # NumPy，数值计算库，用于生成和处理数据
...
# ============================================================================
# 🔑 重点：随机种子就像是"存档点"
#    - 没有种子：每次运行结果都不一样（就像每次玩游戏都从头开始）
#    - 有了种子：每次运行结果一致（就像从存档点继续，一切都一样）
#
# ❓ 思考：为什么需要可复现？
#    答：调试时需要对比结果，论文需要他人验证，生产环境需要稳定性

torch.manual_seed(42)     # 固定 PyTorch 的随机种子
np.random.seed(42)        # 固定 NumPy 的随机种子
# 💡 42 是个"梗"，来自《银河系漫游指南》，代表"生命、宇宙以及一切的答案"
...
# 💡 fc1: 3*8 + 8 = 32 个参数（权重24个 + 偏置8个）
# 💡 fc2: 8*1 + 1 = 9 个参数（权重8个 + 偏置1个）
# 💡 总共: 32 + 9 = 41 个参数
