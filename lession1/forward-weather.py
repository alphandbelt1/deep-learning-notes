...
#
# ═══════════════════════════════════════════════════════════════════════════════
# 【Python list vs NumPy ndarray vs PyTorch Tensor 对比】
# ═══════════════════════════════════════════════════════════════════════════════
#
# 一、内存布局的本质区别
#
#    ┌─────────────────────────────────────────────────────────────────────────┐
#    │  Python list —— 存储的是「指针」，数据分散在内存各处                        │
#    ├─────────────────────────────────────────────────────────────────────────┤
#    │                                                                         │
#    │    list = [1.0, 2.0, 3.0]                                               │
#    │                                                                         │
#    │    list 对象                                                             │
#    │    ┌──────────────────┐                                                 │
#    │    │ ptr[0] ──────────┼──────► PyObject(1.0) 在地址 0x1000              │
#    │    │ ptr[1] ──────────┼──────► PyObject(2.0) 在地址 0x2000  ← 不连续！   │
#    │    │ ptr[2] ──────────┼──────► PyObject(3.0) 在地址 0x3000              │
#    │    └──────────────────┘                                                 │
#    │                                                                         │
#    │    每个元素是一个完整的 Python 对象，包含：                                │
#    │    - 引用计数 (8 bytes)                                                 │
#    │    - 类型指针 (8 bytes)                                                 │
#    │    - 实际数值 (8 bytes for float)                                       │
#    │    单个 float 占用约 24+ bytes！                                         │
#    │                                                                         │
#    └─────────────────────────────────────────────────────────────────────────┘
#
#    ┌─────────────────────────────────────────────────────────────────────────┐
#    │  NumPy ndarray —— 存储「原始数值」，数据连续紧凑                           │
#    ├─────────────────────────────────────────────────────────────────────────┤
#    │                                                                         │
...
#    │    ndarray 对象          连续内存块                                       │
#    │    ┌────────────┐       ┌─────┬─────┬─────┐                             │
#    │    │ shape      │       │ 1.0 │ 2.0 │ 3.0 │  ← 紧密排列！                │
#    │    │ dtype      │──────►│4byte│4byte│4byte│                             │
#    │    │ data_ptr   │       └─────┴─────┴─────┘                             │
#    │    └────────────┘       地址 0x1000 0x1004 0x1008                        │
#    │                                                                         │
#    │    3 个 float32 只占用 12 bytes（vs list 的 72+ bytes）                  │
#    │    内存效率提升 6 倍！                                                    │
#    │                                                                         │
#    └─────────────────────────────────────────────────────────────────────────┘
#
...
#    任务：将 100 万个数字各自平方
#
#    # Python list 方式（逐元素循环）
#    py_list = list(range(1000000))
#    result = [x**2 for x in py_list]       # 约 100ms
#
#    # NumPy 方式（向量化）
#    np_arr = np.arange(1000000)
#    result = np_arr ** 2                   # 约 1ms  ← 快 100 倍！
#
#    为什么 NumPy 快？
#    1. 没有 Python 解释器开销（C 循环 vs Python 循环）
#    2. 连续内存访问，CPU 缓存命中率高
#    3. SIMD 指令并行处理多个数据（一条指令处理 4~8 个 float）
#
# 三、功能对比表
#
#    ┌────────────────┬─────────────┬─────────────┬─────────────┐
#    │     特性       │ Python list │ NumPy array │ PyTorch     │
#    ├────────────────┼─────────────┼─────────────┼─────────────┤
#    │ 元素类型       │ 任意混合    │ 单一类型    │ 单一类型    │
#    │ 内存布局       │ 分散        │ 连续        │ 连续        │
#    │ 数学运算       │ 需要循环    │ 向量化      │ 向量化      │
...
#
#    # NumPy array：+ 是逐元素加法，* 是逐元素乘法
#    np.array([1,2]) + np.array([3,4])  # → array([4, 6])  逐元素加
#    np.array([1,2]) * 3                # → array([3, 6])  逐元素乘
#
#    # PyTorch tensor：同 NumPy
#    torch.tensor([1,2]) + torch.tensor([3,4])  # → tensor([4, 6])
#    torch.tensor([1,2]) * 3                    # → tensor([3, 6])
#
# 五、类型系统对比
#
#    Python list：动态类型，可混合
#        mixed = [1, "hello", 3.14, [1,2]]  # ✓ 合法
#
#    NumPy array：静态类型，必须统一
#        np.array([1, "hello", 3.14])  # → array(['1', 'hello', '3.14'])
#        # 自动转成字符串！因为字符串是"最通用"的类型
#
#    常见 dtype：
#        np.int32, np.int64      → 整数
#        np.float32, np.float64  → 浮点数（深度学习常用 float32）
#        np.bool_                → 布尔
#        np.complex64            → 复数
#
# 六、何时用哪个？
...
#    场景                                   推荐
#    ─────────────────────────────────────────────────────────────
#    存储异构数据（姓名+年龄+分数）          Python list / dict
#    需要频繁增删元素                        Python list
#    数值计算、矩阵运算、统计                NumPy
#    深度学习训练、需要 GPU、需要梯度        PyTorch Tensor
#    数据预处理后喂给神经网络                NumPy → PyTorch
#
# ═══════════════════════════════════════════════════════════════════════════════
